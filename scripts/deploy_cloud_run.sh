...
CONFIG_APP_ENV_KEYS=(
  ...
  TRANSCRIPTION_DISPATCH_URL
  TRANSCRIPTION_DISPATCH_URL_DEBUG
  ...
  TRANSCRIPTION_MAX_ITEMS
  TRANSCRIPTION_MAX_FILE_BYTES
  TRANSCRIPTION_MAX_DURATION_SECONDS
  TRANSCRIPTION_TASK_DEADLINE_SECONDS
  TRANSCRIPTION_TASK_MAX_ATTEMPTS
)
...
echo "Ensure Cloud Tasks queue '${TRANSCRIPTION_QUEUE_NAME:-transcriptions}' exists in ${GCP_REGION}."
echo "Recommended: gcloud tasks queues create ${TRANSCRIPTION_QUEUE_NAME:-transcriptions} --location=${GCP_REGION} --max-concurrent-dispatches=2 --max-dispatches-per-second=1 --max-attempts=${TRANSCRIPTION_TASK_MAX_ATTEMPTS:-5}"
